  ...
  if [[ "$REPORT" = "1" ]]; then
    ...
    for HOST in `cat $FILE`; do
      ...
      if [[ $WEBSCREENSHOT = "1" ]]; then
        cd $LOOT_DIR
        python3 $INSTALL_DIR/bin/webscreenshot.py -r chromium http://$TARGET:80 2> /dev/null > /dev/null &
        python3 $INSTALL_DIR/bin/webscreenshot.py -r chromium https://$TARGET:443 2> /dev/null > /dev/null &
      fi
